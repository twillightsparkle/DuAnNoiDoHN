...
export default function NoblePalaceTayThangLongPage() {
  return (
    <div className="space-y-10">
      ...
      <section className="grid gap-8 rounded-[2rem] border border-amber-200 bg-white p-6 shadow-[0_18px_60px_rgba(120,73,12,0.12)] lg:grid-cols-[1.05fr_0.95fr] lg:p-8">
        ...
        <aside className="space-y-4 rounded-[1.5rem] border border-amber-200 bg-gradient-to-b from-amber-50 to-white p-5">
          <div>
            ...
            <p className="mt-1 text-base leading-7 text-amber-950/85">{noblePalaceTayThangLong.location}</p>
          </div>
          <div>
            <p className="text-sm text-amber-700">Overview</p>
            <p className="mt-1 text-base leading-7 text-amber-950/85">{noblePalaceTayThangLong.overview}</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {noblePalaceTayThangLong.stats.map((stat) => (
      ...
      <section className="rounded-[2rem] border border-amber-200 bg-white p-6 shadow-lg shadow-amber-950/5">
        <div className="mb-6 flex items-center gap-4">
          ...
          <h2 className="text-2xl font-semibold text-amber-950">Core advantages</h2>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          {noblePalaceTayThangLong.highlights.map((item) => (
      ...
      <section className="space-y-5">
        <div className="flex items-center gap-4">
          <div className="h-1 w-24 rounded-full bg-gradient-to-r from-yellow-500 to-amber-700" />
          <h2 className="text-2xl font-semibold text-amber-950">Main product lines</h2>
        </div>
        <div className="grid gap-5 md:grid-cols-2">
          {noblePalaceTayThangLong.featureCards?.map((card) => (
      ...
      <section className="rounded-[2rem] border border-amber-200 bg-gradient-to-r from-amber-950 via-amber-900 to-yellow-700 p-6 text-amber-50 shadow-xl shadow-amber-950/10">
        <div className="grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="space-y-3">
            <p className="text-xs uppercase tracking-[0.24em] text-amber-100/80">Gallery</p>
            <h2 className="text-2xl font-semibold">Project imagery and urban atmosphere</h2>
            <p className="max-w-2xl text-sm leading-6 text-amber-50/85">
              {noblePalaceTayThangLong.cta}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {noblePalaceTayThangLong.gallery.map((image) => (
